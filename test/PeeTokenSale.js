...
contract('PeeTokenSale', function(accounts) {
	var tokenInstance;
	...
	it('initialises the contract with the correct values', function() {
		return PeeTokenSale.deployed().then(function(instance) {
	...
	it('facilitates token buying', function() {
		return PeeToken.deployed().then(function(instance) {
			...
			// Try to buy tokens at incorrect ether value
			return tokenSaleInstance.buyTokens(numberOfTokens, { from: buyer, value: 1 });
		}).then(assert.fail).catch(function(error) {
			assert(error.message.toString().indexOf('revert') >= 0, 'msg.value must equal number of tokens in Wei');
			return tokenSaleInstance.buyTokens(70000, { from: buyer, value: numberOfTokens * tokenPrice })
		}).then(assert.fail).catch(function(error) {
	...
	it('ends token sale', function() {
		return PeeToken.deployed().then(function(instance) {
			...
			assert.equal(balance.toNumber(), 419990, 'returns all unsold tokens to admin');
			// Check that the contract has no balance
			return tokenInstance.balanceOf(tokenSaleInstance.address)
		}).then(function(balance) {
			assert.equal(balance, 0, 'contract is empty of tokens');
